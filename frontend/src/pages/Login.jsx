  ...
  return (
    <div className="grid min-h-screen place-items-center bg-slate-950 p-5">
      <form onSubmit={submit} className="w-full max-w-md rounded-3xl bg-white p-7 shadow-xl">
        <h1 className="text-2xl font-semibold">Footprint</h1>
        <p className="mt-2 text-sm text-slate-500">Find public exposure, understand its risk, and prepare removal requests.</p>
        {mode === "signup" && <input className="mt-6 w-full rounded-xl border p-3" placeholder="Full legal name"
          value={form.fullName} onChange={(e) => setForm({ ...form, fullName: e.target.value })} />}
        <input className="mt-3 w-full rounded-xl border p-3" placeholder="Email" type="email"
          value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
        <input className="mt-3 w-full rounded-xl border p-3" placeholder="Password" type="password"
          value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />
        <button className="mt-4 w-full rounded-xl bg-slate-950 px-4 py-3 font-medium text-white">
          {mode === "signup" ? "Create account" : "Sign in"}
        </button>
        {message && <p className="mt-3 text-sm text-slate-600">{message}</p>}
        <button type="button" onClick={() => setMode(mode === "signup" ? "login" : "signup")} className="mt-5 text-sm text-blue-700">
